...
interface StudyState { revision: number; kanji: ReviewProgress; }

@Injectable({providedIn: 'root'})
export class StudySyncService {
  readonly state = new BehaviorSubject<StudyState>({revision: 0, kanji: {}});
  ...
  initialize(cards: KanjiCard[]): void {
    ...
    this.cards = cards;
    try { this.state.next({revision: 0, kanji: parseProgress(localStorage.getItem(this.key), cards)}); }
    catch {
      this.canCache = false;
  ...
  private merge(remote: StudyState): void {
    ...
    this.state.next({revision: remote.revision, kanji: mergeReviews(this.state.value.kanji, restored)});
    this.cache();
  }

  private cache(): void {
    if (!this.canCache) return;
    try { localStorage.setItem(this.key, JSON.stringify({version: 1, cards: this.state.value.kanji})); }
